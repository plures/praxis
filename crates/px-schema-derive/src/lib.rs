    ...
    if let Data::Struct(data_struct) = &input.data {
        if let Fields::Named(fields) = &data_struct.fields {
            for field in &fields.named {
                ...
                field_schemas.push((field_name, field_desc, inferred_type, field_required, default_value, example, one_of));
            }
        }
    }
    
    // Generate the implementation
    let field_entries = field_schemas.iter().map(|(name, desc, ty, req, def, ex, _one_of)| {
