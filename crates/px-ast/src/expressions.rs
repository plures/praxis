...
pub enum Expr {
    ...
    InlineIf { condition: Box<Expr>, then_val: Box<Expr>, else_val: Box<Expr> },
    /// Binary operation (logic, comparison, arithmetic)
    Binary { left: Box<Expr>, op: BinOp, right: Box<Expr> },
    /// Unary (!, NOT, -)
    Unary { op: UnaryOp, operand: Box<Expr> },
    /// `match subject { pattern => result, ... }`
    Match { subject: Box<Expr>, arms: Vec<ExprMatchArm> },
...
pub enum CodeExpr {
    ...
    InlineIf { condition: Box<CodeExpr>, then_val: Box<CodeExpr>, else_val: Box<CodeExpr> },
    /// Binary operation
    Binary { left: Box<CodeExpr>, op: BinOp, right: Box<CodeExpr> },
    /// Unary (!, -)
    Unary { op: UnaryOp, operand: Box<CodeExpr> },
    /// Function call: `name(args...)`
    Call { name: Ident, args: Vec<CodeExpr>, access_chain: Vec<CodeAccess> },
    /// Variable/path access: `foo.bar[0]`
    Access { base: DottedIdent, chain: Vec<CodeAccess> },
    /// Closure: `|x, y| expr`
    Closure { params: Vec<Ident>, body: Box<CodeExpr> },
