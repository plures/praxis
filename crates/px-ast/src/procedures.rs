...
use crate::common::*;
use crate::expressions::{Expr, CodeExpr};
...
pub enum ProcedureTrigger {
    Periodic { interval: Option<Value> },
    OnWrite { pattern: Option<StringLiteral>, args: Option<Value> },
    OnEvent(StringLiteral),
    Startup,
    BeforeResponse,
    AfterResponse,
    Cron { schedule: Option<Value> },
    Manual,
}
...
pub enum Step {
    ...
    /// `if expr:\n  steps...\nelse:\n  steps...\nend`
    If { condition: Expr, then_steps: Vec<Step>, else_steps: Option<Vec<Step>> },
    /// `match:\n  pattern -> action\nend`
    Match { arms: Vec<MatchArm> },
    /// `when expr:\n  steps...\nend`
    When { condition: Expr, steps: Vec<Step> },
    /// `for $var in collection:\n  steps...\nend`
    For { var: VarRef, collection: Expr, steps: Vec<Step> },
...
pub enum CodeStmt {
    ...
    Assign { target: String, op: AssignOp, value: CodeExpr },
    /// `if expr { ... } else { ... }`
    If { condition: CodeExpr, then_block: CodeBlock, else_clause: Option<ElseClause> },
    /// `for name in expr { ... }`
    For { var: Ident, iter: CodeExpr, body: CodeBlock },
    /// `match expr { pattern => result, ... }`
    Match { subject: CodeExpr, arms: Vec<CodeMatchArm> },
    /// `try { ... } catch name { ... }`
    Try { body: CodeBlock, catch: Option<(Option<Ident>, CodeBlock)> },
    /// `parallel { branch_name: { ... }, ... }`
    Parallel { branches: Vec<(Ident, CodeBlock)> },
    /// `return expr;`
    Return { value: Option<CodeExpr> },
    /// `emit(queue, value);`
    Emit { queue: CodeExpr, value: Option<CodeExpr> },
