...
mod tests {
    ...
    fn test_type_mismatch() {
        ...
        let result = validate(&doc, &schema);
        assert!(!result.is_valid());
        assert!(result.errors.iter().any(|e| e.message.contains("Type mismatch")));
    }
    ...
    fn test_unknown_construct_warning() {
        ...
        assert_eq!(result.warnings.len(), 1);
        assert!(result.warnings[0].message.contains("Unknown construct 'unknown_construct'"));
    }
    ...
    fn test_one_of_validation() {
        ...
        assert!(!result.is_valid());
        assert!(result.errors.iter().any(|e| e.message.contains("must be one of")));
    }
}
